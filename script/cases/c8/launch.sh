...
function parties {
  $POSTGRES_SYSBENCH_DIR/bin/sysbench  --pgsql-db=postgres --pgsql-user=$(whoami) --tables=1 --table-size=1000 --threads=1 --time=115  --percentile=50 --report-interval=1 $POSTGRES_SYSBENCH_DIR/share/sysbench/oltp_update_index.lua run >> $LOG_DIR/c8/front_1/parties.log &
  sleep 1
  ...
  sleep 1
  $POSTGRES_SYSBENCH_DIR/bin/sysbench  --pgsql-db=postgres --pgsql-user=$(whoami) --tables=64 --table-size=1000 --threads=32 --time=300  --percentile=50 --report-interval=1 $POSTGRES_SYSBENCH_DIR/share/sysbench/select_random_points.lua run > $LOG_DIR/c8/front_2/parties.log &
  sleep 1
  N=$(ps -e -T | grep postgre | awk '{print $2}' | sort -h | wc -l)
  N=$((N-31))
  TLIST=$(ps -e -T | grep postgre | awk '{print $2}' | sort -h | tail -n +${N})  
  ...
  sudo ../../comparsion/parties_for_native.py $LOG_DIR/c8/ &
  sleep 90
