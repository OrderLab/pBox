...
def plot(args, relative=False, ratio_only=False, show_normal=True, large=True):
    ...
    retro = df["retro(ms)"]
    retro_normal = normal
    retro_interference = interference
    ...
    series = [normal, interference, psandbox, cgroup, parties, retro]
    labels = ['w/o interference', 'w/ interference', 'pbox', 'cgroup', 'PARTIES', 'Retro']
    colors = ['#a6dc80', '#c00000', '#ffc433', '#98c8df','#66cdaa', '#8ee5ee']
    if not show_normal:
        del series[0]
        del labels[0]
        del colors[0]
